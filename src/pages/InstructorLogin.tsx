...
import { forgotInstructorPassword } from "@/services/instructorAuthApi";
...
const InstructorLogin = () => {
  ...
  const handleForgotPassword = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!resetEmail.trim()) {
      toast({ title: "Email required", description: "Enter your registered email address." });
      return;
    }

    try {
      setIsSubmitting(true);
      const response = await forgotInstructorPassword({ email: resetEmail.trim() });
        ...
        <section className="py-16">
          <div className="container mx-auto px-4">
            <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] items-center">
              ...
              >
                <h1 className="text-2xl md:text-3xl font-heading font-bold text-[#4B1E83]">
                  {isResetMode ? "Reset Password" : "Instructor Login"}
                </h1>
                <div className="mt-6 space-y-5">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
                    <button type="button" className="text-[#4B1E83] hover:underline" onClick={() => setIsResetMode((value) => !value)}>
